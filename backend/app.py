...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:pindrajora@localhost:5432/assign'
...
def create_seller():
    try:
        ...
        newUser = Seller(public_id = str(uuid.uuid4()), email = data['email'], name = data['name'], password = password, address = data['address'], phone = data['phone'])
...
def create_buyer():
    try:
        ...
        newUser = Buyer(public_id = str(uuid.uuid4()), email = data['email'], name = data['name'], password = password, address = data['address'], phone = data['phone'])
        db.session.add(newUser)
        db.session.commit()
    except:
        return jsonify({'message':'some error occured'})
...
def create_item():
    try:
        data = request.get_json()
        newItem = Item(name = data['name'], sid = data['sid'], quantity = data['quantity'], price = data['price'])
...
def create_cart():
    try:
        data = request.get_json()
        newCart = Cart(item_id = data['item_id'], quantity = data['quantity'], buyer_id = data['buyer_id'], seller_id = data['seller_id'])
